/*
  # Système de Gestion de Boutique de Vêtements - Schéma Initial

  1. Nouvelles Tables
    - `users` - Utilisateurs du système (administrateurs et agents)
    - `customers` - Base de données des clients
    - `products` - Catalogue des produits vestimentaires
    - `transactions` - Historique des ventes et locations
    - `rentals` - Gestion spécifique des locations avec dates
    - `rental_calendar` - Calendrier des réservations de produits
    - `payments` - Gestion des paiements et versements
...
CREATE INDEX IF NOT EXISTS idx_rental_calendar_date ON rental_calendar(reserved_date);
...
WHERE p.name = 'Chemise Blanche' AND u.email = 'agent@boutique.com'
LIMIT 1;
